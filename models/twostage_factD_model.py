...
class TwoStageModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        assert (opt.use_multi_class_GAN is False)
        assert (opt.no_cgan is False)
        ...
        if self.isTrain:
            ...
            # D1 and D2 should be the same, no explicit assertion here
    ...
    def backward_D1(self):
        ...
        self.loss_D1_fake = 0
        for netD1 in self.netD1:
            pred_fake = netD1.forward(fake.detach())
    ...
    def backward_D2_binary(self):
        ...
        if 'real_fake' in self.opt.GAN_losses_D2:
            if self.opt.no_cgan:
                ...
                fake_A = self.transform_inverse(fake.narrow(1, 0, self.opt.input_nc))
            num_fake_pairs += 1
            for netD1, netD2 in zip(self.netD1, self.netD2):
                pred_fake = mul(self.transform(netD1.forward(fake_A.detach())), netD2.forward(fake.detach()))
        ...
        if self.opt.no_cgan:
            ...
            real_A = self.transform_inverse(self.real_A)
        self.loss_D2_real = 0
        for netD1, netD2 in zip(self.netD1, self.netD2):
            pred_real = mul(self.transform(netD1.forward(real_A)), netD2.forward(real))
    ...
    def backward_G(self):
        ...
        # loss of G1
        self.loss_G1_GAN = 0
        for netD1, lambda_D1 in zip(self.netD1, self.opt.lambda_D1):
            pred_fake = netD1.forward(self.fake_A)
            if not self.opt.no_logD_trick:
                self.loss_G1_GAN += self.criterionGAN1(pred_fake, True) * lambda_D1
            else:
                self.loss_G1_GAN += -self.criterionGAN1(pred_fake, False) * lambda_D1
        ...
        if 'real_fake' in self.opt.GAN_losses_G2:
            if self.opt.no_cgan:
                ...
                fake_A = self.transform_inverse(self.real_A)
            num_fake_pairs += 1
            for netD1, netD2, lambda_D2 in zip(self.netD1, self.netD2, self.opt.lambda_D2):
                pred_fake = mul(self.transform(netD1.forward(fake_A)), netD2.forward(fake))
                if not self.opt.no_logD_trick:
                    self.loss_G2_GAN += self.criterionGAN2(pred_fake, flipped_label) * lambda_D2
                else:
                    self.loss_G2_GAN += -self.criterionGAN2(pred_fake, False) * lambda_D2
