...
class CGANCausalModel(BaseModel):
    def name(self):
        return 'cGANCausalModel'
    ...
    def initialize(self, opt):
        ...
        # G3 is "anti-labeler"
        self.netLG = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf2, opt.which_model_netG2, opt.norm,
                                       not opt.no_dropout2, n_layers_G=self.opt.n_layers_G2, use_residual=False,
                                       use_fcn=opt.noiseSize2 != 1, noise_nc=opt.noise_nc2,
                                       add_gaussian_noise=opt.add_gaussian_noise, gaussian_sigma=opt.gaussian_sigma,
                                       upsample_mode=opt.upsample_mode2, n_layers_CRN_block=opt.n_layers_CRN_block2,
        ...
        if not self.isTrain or opt.continue_train:
            ...
            self.load_network(self.netLG, 'LG', opt.which_epoch)
            if self.isTrain:
        ...
        if self.isTrain:
            ...
            self.optimizer_LG = torch.optim.Adam([{'name': 'LG', 'params': self.netLG.parameters(), 'lr': opt.lr2}],
                                                 lr=opt.lr, betas=(opt.beta1, 0.999))
        ...
        networks.print_network(self.netLG)
        if self.isTrain:
            for netD in self.netD1:
    ...
    def forward(self):
        ...
        self.fake_B_from_real_A = self.netG1.forward(self.real_A, self.noise1)
        self.fake_B_from_fake_A = self.netG1.forward(self.fake_A, self.noise1)
        self.fake_A_from_real_B = self.netG2.forward(self.real_B, self.noise2)
        self.recon_real_A = self.netG2.forward(self.fake_B_from_real_A, self.noise2)
        self.recon_fake_A = self.netG2.forward(self.fake_B_from_fake_A, self.noise2)
    ...
    def backward_D1(self):
        # Fake
        if self.opt.no_cgan:
            fake = self.fake_pool1.query(self.fake_B_from_real_A)
        else:
            fake = self.fake_pool1.query(torch.cat((self.real_A, self.fake_B_from_real_A), 1))
        self.loss_D_fake = 0
        for netD in self.netD1:
    ...
    def backward_LG(self):
        # Anti-Labeler
        self.loss_LG = torch.nn.BCELoss()((self.recon_fake_A + 1) / 2, (self.fake_A + 1) / 2)  # rescale
        self.loss_LG.backward(retain_graph=True)
    ...
    def backward_G(self):
        ...
        # L1 loss
        self.loss_G_L1 = self.criterionL1(self.fake_B_from_real_A, self.real_B)

        # Labeler
        self.loss_G_LR = torch.nn.BCELoss()((self.fake_A_from_real_B + 1) / 2, (self.real_A + 1) / 2)  # rescale

        # Anti-Labeler
        self.loss_G_LG = torch.nn.BCELoss()((self.recon_fake_A + 1) / 2, (self.fake_A + 1) / 2)  # rescale

        # InfoGAN loss
        self.loss_G_cycle = torch.nn.BCELoss()((self.recon_real_A + 1) / 2, (self.real_A + 1) / 2)  # rescale to [0 1]

        pm = -1 if self.opt.really_CausalGAN else 1
        self.loss_G = self.loss_G_GAN + self.loss_G_L1 * self.opt.lambda_A \
                      + self.loss_G_LR * self.opt.lambda_B + self.loss_G_cycle * self.opt.lambda_A_cycle \
                      + pm * self.loss_G_LG * self.opt.lambda_A_cycle
    ...
    def optimize_parameters(self):
        ...
        self.optimizer_LG.zero_grad()
        self.backward_LG()
        self.optimizer_LG.step()
    ...
    def get_current_visuals(self, save_as_single_image=False):
        ...
        if self.isTrain:
            ...
            return OrderedDict([('real_A', real_A), ('fake_B_real_A', fake_B_from_real_A),
                                ...
                                ('real_B', real_B), ('recon_real_A', recon_real_A), ('recon_fake_A', recon_fake_A)])
        elif save_as_single_image:
            AB = util.tensor2im(torch.cat([self.real_A.data, self.fake_B_from_real_A.data], dim=1))
            return OrderedDict([('AB', AB)])
        else:
            real_A = util.tensor2im(self.real_A.data)
            fake_B = util.tensor2im(self.fake_B_from_real_A.data)
    ...
    def save(self, label):
        ...
        self.save_network(self.netLG, 'LG', label, gpu_ids=self.gpu_ids)
        for netD, n in zip(self.netD1, range(self.n_netD1)):
    ...
    def update_learning_rate(self):
        ...
        for param_group in self.optimizer_G.param_groups:
            if param_group['name'] == 'G1':
                ...
                param_group['lr'] = lr
        for param_group in self.optimizer_LG.param_groups:
            param_group['lr'] = lr2
