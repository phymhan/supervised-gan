...
class CGANCycleModel(BaseModel):
    ...
    def forward(self):
        ...
        # self.fake_A_from_real_B = self.netG2.forward(self.real_B, activation=torch.nn.Sigmoid()) * 2 - 1
    ...
    def backward_D1(self):
        ...
        if not self.opt.train_D_on_fake_fake_pair:
            ...
            if self.opt.no_cgan:
                fake = self.fake_pool1.query(self.fake_B_from_fake_A)
            else:
                fake = self.fake_pool1.query(torch.cat((self.fake_A, self.fake_B_from_fake_A), 1))
    ...
    def backward_G(self):
        ...
        # Segmentation loss
        self.loss_G_CE = torch.nn.BCELoss()((self.fake_A_from_real_B + 1) / 2, (self.real_A + 1) / 2)  # rescale

        # InfoGAN loss
        self.loss_G_real_cycle = torch.nn.BCELoss()((self.recon_real_A + 1) / 2, (self.real_A + 1) / 2)  # rescale
        self.loss_G_fake_cycle = torch.nn.BCELoss()((self.recon_fake_A + 1) / 2, (self.fake_A + 1) / 2)  # rescale

        self.loss_G = self.loss_G_GAN + self.loss_G_L1 * self.opt.lambda_A \
                      + self.loss_G_CE * self.opt.lambda_B \
                      + self.loss_G_real_cycle * self.opt.lambda_A_cycle \
                      + self.loss_G_fake_cycle * self.opt.lambda_A_cycle * self.opt.lambda_fake_cycle
    ...
    def get_current_errors(self):
        err_list = [('G1', self.loss_G.data[0]),
                    ('real_cycle', self.loss_G_real_cycle.data[0]),
                    ('fake_cycle', self.loss_G_fake_cycle.data[0]),
