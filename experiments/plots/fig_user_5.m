...
dataset = {'sgan', 'dsgan', 'joint', 'unsup'};
acc1_mean = zeros(1, 3);
acc1_err = zeros(1, 3);
acc2_mean = zeros(1, 3);
acc2_err = zeros(1, 3);
for j = 1:numel(dataset)
    dd = dir(fullfile(src, dataset{j}, 'x', '*.mat'));
    N = numel(dd);
    acc = zeros(1, N);
    expert = false(1, N);
    for i = 1:numel(dd)
        s = load(fullfile(src, dataset{j}, 'x', dd(i).name));
...
tags = {'SGAN', 'DSGAN', 'Joint', 'Unsup'};

%% re-order
acc1_mean = acc1_mean([2 1 3 4]);
acc1_err = acc1_err([2 1 3 4]);
tags = tags([2 1 3 4]);
...
set(gca, 'Position', [33.3700+20   50.0984+30  192.9750-75*0.5  176.6016])
