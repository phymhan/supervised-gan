...
clear
load feat.mat
datasets = {'train', 'val', 'ours', 'non-ms', 'non-fc'};
% n_test = 50; % nnz(label == 2);

opt = '-s 0 -c 1';

labelMapping = [1, 1, 2, 3, 4];
tags = {'Real', 'FCGAN', 'Non-MS', 'Non-FC'};
...
figure
hold on
acc_mean = cellfun(@mean, accs(2:end));
acc_err = cellfun(@std, accs(2:end));
bar(acc_mean)
...
set(gca, 'Position', [33.3700+30   50.0984  192.9750-75  176.6016])
