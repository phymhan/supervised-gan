...
for j = 1:n_group
    if acc_mean(j) < 0.5
        text(j, acc_mean(j)+acc_err(j)+0.05, sprintf('%.0f%%', acc_mean(j)*100), 'HorizontalAlignment', 'center', 'fontsize', 9);
    else
        text(j, acc_mean(j)-acc_err(j)-0.05, sprintf('%.0f%%', acc_mean(j)*100), 'HorizontalAlignment', 'center', 'fontsize', 9);
    end
end
...
set(gca, 'Position', [33.3700+30   50.0984+30  192.9750+75/2  176.6016])
